...
def perfill(
   ...
   # outputCsvDirBase: str = "/eos/user/l/lcuevasp/2017_Analisys/new_ratefile_studys/output_allCorr/output",
    path: str = "/eos/user/b/benitezj/BRIL/PCC_Run3/SummerLUMPAS22/2022vdM//v2_pcc",
    outputPlotsDirBase: str = "/eos/user/l/lcuevasp/analysis_2022_ashish",                        
    outputCsvDirBase: str = "/eos/user/l/lcuevasp/2023_Analisys/output_bgf.003",
   # path: str = "/eos/user/l/lcuevasp/2017_Analisys/new_ratefile_studys/output_allCorr/analysed_data", 
    figureExtension: str = "jpg",
    bcidStart: int = 0,
    bcidEnd: int = 3564,
    xlim: list = None,
    refDet: str = "HFET",
    ...
    if peakNormal:
        ...
        if SigNormal:
            ...
            for corr in sorted(list(corrs.keys()), key=len):
                ...
                scanList = sorted(list(SigDict[corrRead].keys()), key=lambda name: int(name[0:4] + name[13:17]))
                ...
                for f in scanList:
                    ...
                    for det in dets:
                        ...
                        if peak2peakCorr:
                            ...
                            sigVis = np.pi * CX * CY * (PX + PY)#*1.0098  ### <-- XY Corr 
                            sigVisErr = np.pi * np.sqrt(
                                (CY * (PX + PY) * CXErr) ** 2 + (CX * (PX + PY) * CYErr) ** 2 + (CX * CY * PXErr) ** 2 + (CX * CY * PYErr) ** 2)#*1.0098 ### <-- XY Corr
                         #   print("aqui:",sigVis)
                        ## Apply xy correction
                        if corr[-len(XY_CORR) :] == XY_CORR:
...
#################################################################################################
                ...
                bx =["BCID 282","BCID 822","BCID 2944","BCID 3123","BCID 3302"]                                                               
                ...
                sv = np.array([])                                                                         
                sverr=np.array([])                                                                                        
                for scan_name, scan_data,h in zip(data_scans.keys(),data_scans.values(),np.arange(1,6)):                       
                    ...
                    for x,y,z,c,b in zip(posi[str(h)],sigVis,sigVisErr,colors,bx):                                
                        ...
                        if scan_name=="8381_11Nov22_004152_11Nov22_010424":
                            print("x,y",x,y)                                        
                ...
                plt.fill_between( [plt.xlim()[0],plt.xlim()[1]], y1=[yMean_z-yzstd,yMean_z-yzstd], y2=[yMean_z+yzstd,yMean_z+yzstd], color=colorList[cc],alpha=0.3)
                labels = ["vdM1","BI1","BI2","vdM3","vdM4"]
                xaxis = [ i+1 for i in range(len(labels)) ]       
                plt.xticks(xaxis, labels)
                plt.legend(loc='center right', bbox_to_anchor=(0.7, 0.3), fontsize=8)                 
                ipt.makeCMSPreli(year="Fill 8381   2022 ",fill=fill,energy=energy)              
                ...
                plt.grid(True)                                                                             
                plt.savefig('/eos/user/l/lcuevasp/analysis_2022_ashish/8381/sigmavis/allscans/test/all_scans_BCID.png') 
        ...
        if SigAll:
            ...
            for corr in sorted(list(corrs.keys()), key=len):
                ...
                for det in dets:
                    ...
                    sigmaVisPerScan = []
                    sigmaVisErrPerScan = []
                    scanList = sorted(list(SigAllDict[corrRead][det].keys()), key=lambda name: int(name[0:4] + name[13:17]))
                    ...
                    for f in scanList:
                       ...
                       ## print("este es",sigVis,sigVisErr) 
                        ## Fit a constant to get sigma vis for the given scan
                    ...
                    sigmaVisPerScan = np.array(sigmaVisPerScan)#*1.0098   #   <--1.0098 refers to XY correction
                    sigmaVisErrPerScan = np.array(sigmaVisErrPerScan)#*1.0098 
                 ...
                 #   plt.ylim(3290,3380)#HFET
                   ...
                   # print("RMS:",np.sqrt(1/len(sigmaVisPerScan)*np.sum((sigmaVisPerScan)**2))
                    ...
                    plt.fill_between( [plt.xlim()[0],plt.xlim()[1]], y1=[sigmaVis-sv_std,sigmaVis-sv_std], y2=[sigmaVis+sv_std,sigmaVis+sv_std], color=colorList[cc],alpha=0.3)
                    ...
                    va="top",
                    fontsize=12,
                        s="$\sigma_{vis}$ = %.2f $\pm$ %.2f (stat) mb\n RMS = %.2f mb" % (sigmaVis, sigmaVisErr,sv_std),
                   ...
                   # ipt.makeTitle(ipt.makeTitleText(var="$\sigma_{vis}$", fit=fit, corr=corrs[corr], det=det))
                    ...
                    # Make CMS preliminary, year, fill
                    ipt.makeCMSPreli(year="Fill 8381   "+year, fill=fill, energy= energy+" TeV")
...
def main():
   ...
   # filedict["6868_30Jun18_151040_30Jun18_153416"] = "vdM2"
   # filedict["6868_41Jul18_002854_01Jul18_005345"] = "Img1"
   # filedict["6868_41Jul18_012042_01Jul18_014645"] = "Img2"
   # filedict["6868_41Jul18_030039_01Jul18_032641"] = "Img3"
