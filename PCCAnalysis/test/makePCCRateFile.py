...
def GetCount(data,DataType,BCID):

    #if DataType=="VTX":
    #    count=data.nVtx
    #else:
    count=data.GetLeaf("BXid_%i" % BCID).GetValue()
...
# typically, PCC data is available only for 5 out of all colliding bx
    ...
    for (i, data) in enumerate(pcc_tree):
        ...
        for bx in PCC_BCID:
            num_clus = GetCount(data,DataType,bx)
            #print data.timeStamp, data.scanIndex, data.pointIndex, num_clus
            clusters[data.scanIndex][data.pointIndex][bx].append(num_clus)
