...
class:   CorrPCCProducerReReco.cc
...
class CorrPCCProducerReReco : public DQMOneEDAnalyzer<edm::one::WatchLuminosityBlocks> {
public:
  explicit CorrPCCProducerReReco(const edm::ParameterSet&);
  ~CorrPCCProducerReReco() override;
...
//--------------------------------------------------------------------------------------------------
CorrPCCProducerReReco::CorrPCCProducerReReco(const edm::ParameterSet& iConfig) {
  pccSrc_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoParameters").getParameter<std::string>("inLumiObLabel");
  prodInst_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoParameters").getParameter<std::string>("ProdInst");
  approxLumiBlockSize_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoParameters").getParameter<int>("approxLumiBlockSize");
...
//--------------------------------------------------------------------------------------------------
CorrPCCProducerReReco::~CorrPCCProducerReReco() {}

//--------------------------------------------------------------------------------------------------
// This method builds the single bunch response given an exponential function (type 2 only)
void CorrPCCProducerReReco::makeCorrectionTemplate() {
...
float CorrPCCProducerReReco::getMaximum(std::vector<float> lumi_vector) {
  float max_lumi = 0;
...
void CorrPCCProducerReReco::estimateType1Frac(std::vector<float> uncorrPCCPerBX, float& type1Frac) {
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReReco::evaluateCorrectionResiduals(std::vector<float> corrected_tmp_) {
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReReco::dqmEndRun(edm::Run const& runSeg, const edm::EventSetup& iSetup) {
...
DEFINE_FWK_MODULE(CorrPCCProducerReReco);
