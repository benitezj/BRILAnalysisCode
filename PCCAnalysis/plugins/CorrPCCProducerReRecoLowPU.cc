...
class:   CorrPCCProducerReRecoLowPU.cc
...
class CorrPCCProducerReRecoLowPU : public DQMOneEDAnalyzer<edm::one::WatchLuminosityBlocks> {
public:
  explicit CorrPCCProducerReRecoLowPU(const edm::ParameterSet&);
  ~CorrPCCProducerReRecoLowPU() override;
...
//--------------------------------------------------------------------------------------------------
CorrPCCProducerReRecoLowPU::CorrPCCProducerReRecoLowPU(const edm::ParameterSet& iConfig) {
  pccSrc_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<std::string>("inLumiObLabel");
  prodInst_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<std::string>("ProdInst");
  approxLumiBlockSize_ =
      iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<int>("approxLumiBlockSize");
  type1Frac = iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<double>("type1Frac");
  type2_a_ = iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<double>("type2_a");
  type2_b_ = iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<double>("type2_b");
  run_ = iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<int>("run");
  minimumNumberOfEvents = iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<int>("minimumNumberOfEvents");
  applyPedestal_ = iConfig.getParameter<edm::ParameterSet>("CorrPCCProducerReRecoLowPUParameters").getParameter<bool>("applyPedestal");
...
//--------------------------------------------------------------------------------------------------
CorrPCCProducerReRecoLowPU::~CorrPCCProducerReRecoLowPU() {}

//--------------------------------------------------------------------------------------------------
// This method builds the single bunch response given an exponential function (type 2 only)
void CorrPCCProducerReRecoLowPU::makeCorrectionTemplate() {
...
float CorrPCCProducerReRecoLowPU::getMaximum(std::vector<float> lumi_vector) {
  float max_lumi = 0;
...
void CorrPCCProducerReRecoLowPU::estimateType1Frac(std::vector<float> uncorrPCCPerBX, float& type1Frac) {
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::evaluateCorrectionResiduals(std::vector<float> corrected_tmp_) {
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::calculateCorrections(std::vector<float> uncorrected,
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::beginLuminosityBlock(edm::LuminosityBlock const& lumiSeg, const edm::EventSetup& iSetup) {

}

//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::endLuminosityBlock(edm::LuminosityBlock const& lumiSeg, const edm::EventSetup& iSetup) {
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::dqmEndRun(edm::Run const& runSeg, const edm::EventSetup& iSetup) {
  // TODO: why was this code not put here in the first place?
  dqmEndRunProduce(runSeg, iSetup);
}

//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::dqmEndRunProduce(edm::Run const& runSeg, const edm::EventSetup& iSetup) {


}

//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::resetBlock() {
  for (unsigned int bx = 0; bx < LumiConstants::numBX; bx++) {
    totalLumiByBX_[bx] = 0;
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& iRun, edm::EventSetup const& context) {
...
//--------------------------------------------------------------------------------------------------
void CorrPCCProducerReRecoLowPU::endJob() {
...
DEFINE_FWK_MODULE(CorrPCCProducerReRecoLowPU);
