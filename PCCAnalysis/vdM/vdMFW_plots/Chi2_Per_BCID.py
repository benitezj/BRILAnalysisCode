...
def plot_SigVis_graph_bcid(detector, tag, correction, fitfunction, fill=6016, separatemod4= False):
   ...
   # fig, ax = plt.subplots(figsize=(9, 6))
    ...
    if not separatemod4:
        ...
        for iscan, scan in enumerate(scanorder[fill]):
           ...
           # mask = (temp["Func"] == "CS") | ((sigmaVis_df["SigmaVis_e"] > 0.01 * sigmaVis_df["SigmaVis"])
            ...
            mask =                           ( (temp["fitStatus_X"] < 2) & (temp["fitStatus_Y"] < 2))
                                               #& ((temp["chi2_X"] / temp["ndof_X"]) < 5) & ((temp["chi2_Y"] / temp["ndof_Y"]) < 5))
            ...
            if "XY" in correction:
                ...
                ye = (temp["chi2_Y"] / temp["ndof_Y"]).values
                y2 = (temp["chi2_Y"] / temp["ndof_Y"]).values
               # ye = (temp["chi2_YErr"] / temp["ndof_YErr"]).values
            x = (temp["BCID"]).values
            print(y)
            print(ye)
            print(x)
        ...
        #    ye = 0
            ...
            label = scanlabels[scan]+"_X"
            labelY=scanlabels[scan]+"_Y"
            color = next(ax._get_lines.prop_cycler)['color']
            ax.errorbar(x+iscan*20, y, yerr=None, fmt="o", markersize=6,color=color, label=label)
            ax.errorbar(x+iscan*20, y2, yerr=None, fmt="s", markersize=6,color=color, label=labelY)#,markerfacecolor='none', markeredgecolor=color)
            mean_y = np.mean(y)
            mean_y2 = np.mean(y2)
    ...
    means_y=np.round(np.mean(means_y),3)
    means_y2=np.round(np.mean(means_y2),3)
    
    means="mean_X:"+str(means_y)+"\nmean_Y:"+str(means_y2) 
    ax.text(0.03, 0.84, means,                                                                                                
  ...
  #  ax.yaxis.offsetText.set_visible(False)
   ...
   # fig.subplots_adjust(bottom=0.15, left=0.2)
    fig.savefig("/eos/user/l/lcuevasp/output_ppRef/plots/Chi2_XY_10299_Alca13bcids_Poly4G.png", dpi=400)
    #if not normalize:
