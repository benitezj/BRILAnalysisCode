                ...
                for f in scanList:
                    ...
                    for det in dets:
                        ...
                        if peak2peakCorr:
                            ...
                            sigVis = np.pi * CX * CY * (PX + PY)*1.0098/1000  ### <-- XY Corr 
                            sigVisErr = np.pi * np.sqrt(
                                (CY * (PX + PY) * CXErr) ** 2 + (CX * (PX + PY) * CYErr) ** 2 + (CX * CY * PXErr) ** 2 + (CX * CY * PYErr) ** 2)*1.0098/1000 ### <-- XY Corr
                         #   print("aqui:",sigVis)
                        ## Apply xy correction
                        if corr[-len(XY_CORR) :] == XY_CORR:
...
#                print(data_scans)
               ...
               # plt.legend(loc='upper right', bbox_to_anchor=(1.0, 0.5), fontsize=10) 
                ...
                plt.grid(True)                                                                             
                plt.savefig('/eos/user/l/lcuevasp/analysis_2022_ashish/8381/sigmavis/allscans/all_scans_BCID.png') 
