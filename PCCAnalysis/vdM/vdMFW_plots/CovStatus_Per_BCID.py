...
sigmaVis_df = pd.read_csv("/eos/user/l/lcuevasp/output_ppRef/10299_SigmaVis_Gated.csv")
...
def plot_SigVis_graph_bcid(detector, tag, correction, fitfunction, fill=6016, separatemod4= False):
   ...
   # fig, ax = plt.subplots(figsize=(9, 6))
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.set_xlabel("BCID")
    ax.set_ylabel("CapSigma_X [mm]")
    scale = 1
    if detector=="pcchd5":
        ax.set_ylabel(r"CovStatus_X")
        scale = 1
    ...
    if not separatemod4:
        ...
        for iscan, scan in enumerate(scanorder[fill]):
           ...
           # mask = (temp["Func"] == "CS") | ((sigmaVis_df["SigmaVis_e"]>0.01*sigmaVis_df["SigmaVis"])& ((temp["fitStatus_X"] < 2) & (temp["fitStatus_Y"] < 2 )& (temp["fitStatus_Y"] < 2) & ((temp["chi2_X"]/temp["ndof_X"]) < 3.5) & ((temp["chi2_Y"]/temp["ndof_Y"]) < 3.5))
            mask = (temp["Func"] == "CS") | ((sigmaVis_df["SigmaVis_e"] > 0.01 * sigmaVis_df["SigmaVis"]))
                                           ...
                                           #  &(temp["fitStatus_X"] < 2) & (temp["fitStatus_Y"] < 2) & ((temp["chi2_X"] / temp["ndof_X"]) < 5) & ((temp["chi2_Y"] / temp["ndof_Y"]) < 5))
           # print("bcids excluded:", temp[~mask].shape[0])                     
            ...
            if "XY" in correction:
                ...
                y = (temp["covStatus_X"]).values
            ye = (temp["CapSigmaErr_X"]).values
            x = (temp["BCID"]).values
            print(y)
            print(ye)
            print(x)
        ...
        #    ye = 0
            ...
            ax.errorbar(x+iscan*20, y, yerr=None, fmt="o", markersize=6, label=label)
        ...
        for j in range(4):
            ...
            for scan in scanorder[fill][0:]:
                ...
                if "XY" in correction:
                    ...
                    y.extend( (temp["covStatus_X"]).values.tolist() )
                ye.extend( (temp["CapSigmaErr_X"]).values.tolist() )
           ...
           # ax.errorbar(x, y, yerr=yerr, fmt="o", markersize=6, label=label)
            ax.plot(x, y, "o", markersize=6, label=label)             
        print(det, fill)    
  ...
  #  if offset:  # si existe offset
   ...
   # fig.subplots_adjust(bottom=0.15, left=0.2)
    fig.savefig("/eos/user/l/lcuevasp/output_ppRef/plots/CovSatusX_10299_Gated_Per_BCID_QG.png", dpi=400)
    #if not normalize:
