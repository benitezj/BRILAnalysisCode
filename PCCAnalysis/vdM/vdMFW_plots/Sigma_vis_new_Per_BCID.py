...
def plot_SigVis_graph_bcid(detector, tag, correction, fitfunction, fill=6016, separatemod4= False):
    ...
    if not separatemod4:
        ...
        for iscan, scan in enumerate(scanorder[fill]):
           ...
           # mask = (temp["Func"] == "CS") | ((sigmaVis_df["SigmaVis_e"]>0.01*sigmaVis_df["SigmaVis"])& ((temp["fitStatus_X"] < 2) & (temp["fitStatus_Y"] < 2 )& (temp["fitStatus_Y"] < 2) & ((temp["chi2_X"]/temp["ndof_X"]) < 3.5) & ((temp["chi2_Y"]/temp["ndof_Y"]) < 3.5))
            mask =(temp["Func"] == "CS")|((temp["fitStatus_X"] < 2) & (temp["fitStatus_Y"] < 2) & ((temp["chi2_X"] / temp["ndof_X"]) < 5) & ((temp["chi2_Y"] / temp["ndof_Y"]) < 5))# ((sigmaVis_df["SigmaVis_e"] > 0.01 * sigmaVis_df["SigmaVis"])
            ...
            if "XY" in correction:
                ...
                y = (temp["SigmaVis"]).values
            ye = (temp["SigmaVis_e"]).values
            x = (temp["BCID"]).values
            print(y)
            print(ye)
            print(x)
        ...
        for j in range(4):
            ...
            for scan in scanorder[fill][0:]:
                ...
                if "XY" in correction:
                    ...
                    y.extend( (temp["SigmaVis"]).values.tolist() )
                ye.extend( (temp["SigmaVis_e"]).values.tolist() )
    ...
    fig.savefig("/eos/user/l/lcuevasp/output_ppRef/plots/Sigma_vis_10299_Alca13bcids_Per_BCID_QG.png", dpi=400)
    #if not normalize:
