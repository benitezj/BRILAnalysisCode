...
void compare_old_reprocessed_data(){
  ...
  for (unsigned int j=0;j<run_number.size();j++){
    ...
    CSV=TString("/eos/user/a/asehrawa/BRIL-new/PCC_reprocess_2018A_newveto/ZeroBias/Run2018A_ZB/")+run_number.at(j)+".csv";
    CSV1=TString("/eos/user/b/benitezj/BRIL/PCC/ZeroBias/AlCaLumiPixels_21Sep2018VdmVeto/Run2018A/")+run_number.at(j)+".csv";
  ...
  C.Print(Path+TString("csv_file_lumi_comparison_RunA_ratio")+".png");
