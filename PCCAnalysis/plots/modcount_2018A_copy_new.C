...
void modcount_2018A_copy_new() {
  ...
  TString Path11 = "/eos/user/a/asehrawa/modcountgraphs_goodmodules/A/"; 
  TCanvas C2("C2");
  C2.cd();
  gStyle->SetOptStat(1111111);
  ...
  std::vector<int> run_number_all = {315252, 315255, 315257, 315258, 315259, 315264, 315265, 315267, 315270, 315322, 315339, 315357, 315361, 315363, 315365, 315366, 315420, 315488, 315489, 315490, 315506, 315509, 315510, 315512, 315533, 315543, 315555, 315556, 315557, 315587, 315640, 315641, 315642, 315644, 315645, 315646, 315647, 315648, 315689, 315690, 315702, 315703, 315704, 315705, 315713, 315721, 315741, 315764, 315770, 315784, 315785, 315786, 315787, 315788, 315789, 315790, 315800, 315801, 315840, 315973, 315974, 316058, 316059, 316060, 316061, 316062, 316082, 316095, 316109, 316110, 316111, 316112, 316113, 316114, 316151, 316153, 316186, 316187, 316199, 316200, 316201, 316202, 316216, 316217, 316218, 316219, 316239, 316240, 316241, 316260, 316271, 316361, 316362, 316363, 316377, 316378, 316379, 316380, 316455, 316456, 316457, 316469, 316470, 316472, 316477, 316478, 316505, 316569, 316590, 316613, 316615, 316664, 316665, 316666, 316667, 316700, 316701, 316702, 316715, 316716, 316717, 316718, 316719, 316720, 316721, 316722, 316723, 316758, 316766, 316797, 316798, 316799, 316808, 316820, 316839, 316851, 316876,316877, 316878, 316879, 316928, 316944, 316985, 316993, 316994, 316995};

  //std::vector<int> run_number_all = {315689};
  
  std::cout<<run_number_all.size()<<std::endl;
    ...
    for (unsigned int k=0;k<run_number_all.size();k++){
      ...
      std::vector<int> run_number_A={run_number_all.at(k)};
      //readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/Run2018A_vetolist_second_iteration.txt");
      readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/veto_RunA_0p02_3sigma_weight_cut.txt");
      ...
      f1.open("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/veto_RunA_0p02_3sigma_weight_cut.txt");
	...
	f1.get(c);
      ...
      for (unsigned int i=0;i<1856;i++){
	histo_counts1[i]=new TH2F(TString("Histo_counts1")+i+"_"+k,"", 1400,0,70000,300,0,0.016);
      }
      
      int modid2[1856];
      for (unsigned int j=0;j<run_number_A.size();j++){
	TString Path = "/eos/user/a/asehrawa/BRIL-PCC_25Sep2021/ZeroBias/Run2018A"; 
	TString infile=Path+"/"+run_number_A.at(j)+".csv"; 
	//std::cout<< run_number_A.at(j)<<std::endl;  
	...
	myfile.close();    
      ...
      for (unsigned int i=0;i<1856;i++){
	if(MODVETO[modid[i]]==0){ 
	  ProjY3[i] = histo_counts1[i]->ProjectionY(TString("Projection3_")+i+"_"+k,0,1400);
      ...
      g14=new TGraph();
      //TH1D *h1;
      h1=new TH1D(TString("histo1D")+"_"+k, "weights (A-B)/B",100,-0.05,0.05);
      for (unsigned int i=0;i<1856;i++){
	...
	if(MODVETO[modid1[i]]==0){
      ...
      //for (unsigned int i=0;i<1856;i++){
      //if(MODVETO[modid1[i]]==0){
      //  if(((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean())<6.97110e-04-3*2.70604e-03 || ((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean())>6.97110e-04+3*2.70604e-03){    
	    //std::cout<<modid1[i]<<std::endl;                            
      //  }
      //}
      //}
	...
	if(MODVETO[modid[i]]==0){
	  if(LY[modid[i]]!=0 && BPIX_Z[modid[i]] < 0){
	    dw_z_negative_barrel[i]=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
	    dw_z_negative_barrel_allmodules+=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
	    //std::cout<<dw_z_negative_barrel[i]<<std::endl;
	  }
	  if(LY[modid[i]]!=0 && BPIX_Z[modid[i]] > 0){
	    dw_z_positive_barrel[i]=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
	    dw_z_positive_barrel_allmodules+=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
	    //std::cout<<dw_z_positive_barrel[i]<<std::endl;
	  }
	  if(LY[modid[i]]==0 && FPIX_Z[modid[i]] < 0){
	    dw_z_negative_endcap[i]=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
	    dw_z_negative_endcap_allmodules+=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
	    //std::cout<<dw_z_negative_endcap[i]<<std::endl;
	  
	  }
	  if(LY[modid[i]]==0 && FPIX_Z[modid[i]] > 0){
	    dw_z_positive_endcap[i]=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
	    dw_z_positive_endcap_allmodules+=((ProjY3[i]->GetMean())-(ProjY4[i]->GetMean()))/(ProjY4[i]->GetMean());
    ...
    g11->SetTitle(TString("Run2018A (Black)") +" & "+ TString("Run2018B (Red)"));
    ...
    g13->GetYaxis()->SetTitle("module weights (A-B)/B");
    ...
    g14->GetYaxis()->SetTitle("module weights (A-B)/B");
    h1->GetXaxis()->SetTitle("weights (A-B)/B");
    h1->GetYaxis()->SetTitle("entries");
    g11->Draw("AP");
    g12->Draw("PSAME");
    C2.Print(Path11+"Graphs_cut_totcount/"+"module_weight_A_B_testing"+".png");

    g13->Draw("AP");
    C2.Print(Path11+"Graphs_cut_totcount/"+"module_weight_(A-B)_B_testing"+".png");
    ...
    l10->SetLineWidth(3);
    l10->Draw("SAME"); 
    C2.Print(Path11+"Graphs_cut_totcount/"+"module_weight_(A-B)_A_vs_zcoordinate_testing"+".png");
    
    h1->Draw();
    C2.Print(Path11+"Graphs_cut_totcount/"+"module_weight_(A-B)_B_histogram_testing"+".png");
