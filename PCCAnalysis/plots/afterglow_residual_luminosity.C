...
void afterglow_residual_luminosity() {
  ...
  TH2F *fhist = new TH2F("fhist","",1000,0,30,100,-0.002,0.002);
  fhist->GetXaxis()->SetTitle("sbil (Hz/ub)");
  //fhist->GetYaxis()->SetTitle("Type1 residual fraction");
  fhist->GetYaxis()->SetTitle("Type2 residual");
  fhist->SetMarkerColor(kRed);
  ...
  for(int k = 0; k < run.size(); k++){
    ...
    residualvsinstlumi[k]->SetMarkerColor(2);
    residualvsinstlumi[k]->SetMarkerSize(1);
  ...
  cf->Print(outpath+"residualtype2vsinstlumi_"+".png");
