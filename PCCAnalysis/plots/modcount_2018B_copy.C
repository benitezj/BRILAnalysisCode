...
void modcount_2018B_copy() {
  ...
  readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/test/veto_lateRunD_lowcut_tight.txt");
  ...
  histo_L2=new TH2F("Histo_Layer2","",200,0,70000,300,0,0.8);
  histo_L3=new TH2F("Histo_Layer3","",200,0,70000,300,0,0.35);
  histo_L4=new TH2F("Histo_Layer4","",200,0,70000,300,0,0.35);
  histo_D1S1=new TH2F("Histo_Disk1S1","",200,0,70000,300,0,0.35);
  histo_D2S1=new TH2F("Histo_Disk2S1","",200,0,70000,300,0,0.35);
  histo_D3S1=new TH2F("Histo_Disk3S1","",200,0,70000,300,0,0.8);
  histo_D1S2=new TH2F("Histo_Disk1S2","",200,0,70000,300,0,0.35);
  histo_D2S2=new TH2F("Histo_Disk2S2","",200,0,70000,300,0,0.35);
  histo_D3S2=new TH2F("Histo_Disk3S2","",200,0,70000,300,0,0.35);
  
  for (unsigned int i=0;i<1856;i++){
    gr[i]=new TGraph();
    histo_counts[i]=new TH2F(TString("Histo_counts")+i,"", 700,0,70000,300,0,0.016);
  }
  
  int modid[1856];
  for (unsigned int j=0;j<run_number.size();j++){
    TString Path = "/eos/user/a/asehrawa/BRIL-PCC_25Sep2021/ZeroBias/Run2018B";
    //TString Path = "/eos/user/a/asehrawa/BRIL-PCC_newveto_2Oct2021/ZeroBias/Run2018B";
	...
	if(MODVETO[modid[i]]==0){                                                                                
      ...
      for (unsigned int i=0;i<1856;i++){
	if(totcount > 0){
	  //if(MODVETO[modid[i]]==0 && m_count[i]>0){
	  if(MODVETO[modid[i]]==0){
  ...
  for (unsigned int i=0;i<1856;i++){
	...
	histo_counts[i]->SetTitle(modid[i]+ TString(" BPIX layer ") + LY[modid[i]] +" ladder " + LD[modid[i]] + " MD " + MD[modid[i]]);
      ...
      histo_counts[i]->GetYaxis()->SetRangeUser(0, 0.025);      
	...
	gr3->SetPoint(gr3->GetN(), i, BPIX_Z[modid[i]]);
      ...
      //gr8->GetYaxis()->SetRangeUser(0, 0.16);
      //gr8->GetYaxis()->SetNdivisions(80);
      //gr8->GetYaxis()->SetLabelSize(0.02);
      gr8->GetXaxis()->SetRangeUser(0, 1900);
      gr8->GetYaxis()->SetTitle("ProjectionY RMS/Mean");
      gr8->GetXaxis()->SetTitle("module number index");

      if(ProjY1[i]->GetMean()!=0){
	gr8->SetPoint(gr8->GetN(), i, (ProjY1[i]->GetRMS())/(ProjY1[i]->GetMean()));
      }

      if((ProjY1[i]->GetRMS())/(ProjY1[i]->GetMean())>=0.03){

	std::cout<<modid[i]<<std::endl;
      }

      ProjY[i]->GetXaxis()->SetRangeUser(0, 0.005);
      if(LY[modid[i]]==0){
	...
	ProjY[i]->SetTitle(modid[i]+ TString(" BPIX layer ") + LY[modid[i]] +" ladder " + LD[modid[i]] + " MD " + MD[modid[i]]);
      ...
      //C.Print(Path1 + "Graph1D_test1/"+hist_name3);
  ...
  l->SetLineStyle(2);                                                                                                                       
  l->Draw("SAME"); 
  C.Print(Path1+"Graph1D_test1/"+"RMS_Mean"+".png"); 
  ...
  //C.Print(Path1+"Graph1D_test1/"+"Histo_stability_combined"+".png");  
  //C.Print(Path1+"Graph1D_test1/"+"Histo_stability_newveto_combined"+".png");
  ...
  P_L2->GetYaxis()->SetTitle("Lumi fraction (per 300 lumi section)");
  ...
  //auto* legend = new TLegend(0.7,0.35,0.85,0.8);
  //legend->AddEntry(P_L2,"B1","l");
  //legend->AddEntry(P_L3,"B2","l");
  //legend->AddEntry(P_L4,"B3","l");
  //legend->AddEntry(P_D1S1,"F1S1","l");
  //legend->AddEntry(P_D2S1,"F2S1","l");
  //legend->AddEntry(P_D3S1,"F3S1","l");
  //legend->AddEntry(P_D1S2,"F1S2","l");
  //legend->AddEntry(P_D2S2,"F2S2","l");
  //legend->AddEntry(P_D3S2,"F3S2","l");
  //legend->SetFillColor(0);
  //legend->SetLineColor(0);
  //legend->SetFillColor(0);
  //legend->Draw("SAME");
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_combined"+".png");
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_newveto_combined"+".png");
    
  //P_L2->Draw();  
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_L2"+".png"); 
  //P_L3->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_L3"+".png");
  //P_L4->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_L4"+".png");
  //P_D1S1->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_D1S1"+".png");
  //P_D2S1->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_D2S1"+".png");
  //P_D3S1->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_D3S1"+".png");
  //P_D1S2->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_D1S2"+".png");
  //P_D2S2->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_D2S2"+".png");
  //P_D3S2->Draw();
  //C.Print(Path1+"Graph1D_test1/"+"ProfileX_D3S2"+".png");
