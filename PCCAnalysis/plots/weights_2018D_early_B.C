...
void weights_2018D_early_B() {
  ...
  //std::vector<int> run_number={323475, 323487, 323488, 323492, 323493, 323495, 323524, 323525, 323526};
  ...
  //readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/Run_D1_veto.txt");
  //readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/Run_D2_veto.txt");
  //readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/RunD_early_veto_latest.txt");
  //readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/D1_veto_first_iteration.txt");
  //readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/D2_vetolist_first_iteration.txt");
  ...
  //readModVeto("/afs/cern.ch/user/a/asehrawa/CMSSW_10_2_2/src/BRILAnalysisCode/PCCAnalysis/plots/empty_veto_file.txt");
  ...
  std::cout<<"sum of weights is "<<weights_sum<<std::endl;
  ...
  //std::vector<int> run_number1 = {318667, 318669, 318670, 318675, 318712,318714, 318733, 318734};
  ...
  for (unsigned int j=0;j<run_number1.size();j++){
	...
	if(totcount1 > 100000000){
	  if(MODVETO[modid1[i]]==0){
	    ...
	    //std::cout<<LS1<<"  "<<m_count1[i]/totcount1<<std::endl;
	  }
	}
      }
    }
  ...
  //std::cout<<i<<"  "<<((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<<std::endl;
  //  if(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<-0.01 || ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())>0.01) {                                                                                                                       //std::cout<<modid1[i]<<std::endl;                                                                                                      
  // }                                  
  //}
  //}
  //}
  ...
  //std::cout<<g3->GetN()<<" "<<i<<" "<<((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<<std::endl;                
//    h->Fill(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
//    if(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<1.16553e-04-3*2.25602e-03 || ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())>1.16553e-04+3*2.25602e-03) {                                                                            
  ...
  //std::cout<<i<<" "<<((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<<std::endl;                
  //h->Fill(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
  //if(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<-6.64840e-04-3*2.97974e-03 || ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())>-6.64840e-04+3*2.97974e-03) {
  //	std::cout<<modid1[i]<<std::endl;
  //}
  //}
  //}
  ...
  for (unsigned int i=0;i<1856;i++){
    if(MODVETO[modid1[i]]==0){
      g3->SetPoint(g3->GetN(), i, ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
      //std::cout<<modid[i]<<" "<<((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<<std::endl;                          
      std::cout<<modid[i]<<std::endl;
      h->Fill(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
      //if(LY[modid1[i]]!=0) {
      //g4->SetPoint(g4->GetN(), BPIX_Z[modid1[i]], ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
      //}
      //if(LY[modid1[i]]==0) {
      //g4->SetPoint(g4->GetN(), FPIX_Z[modid1[i]], ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
      //}

      //std::cout<<modid1[i]<<"  "<<((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<<std::endl;      
      //g4->SetPoint(g4->GetN(), i, BPIX_Z[modid1[i]]);
      //if(LY[modid1[i]]!=0) {
      //g4->SetPoint(g4->GetN(), FPIX_Z[modid1[i]], ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
      //}
      //if(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())< 1.20580e-03-3*4.98387e-03 || ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())> 1.20580e-03+3*4.98387e-03 ) {                                                                       
      //std::cout<<modid1[i]<<std::endl;                                                                                                   
      //}
    }
  }
  ...
  //std::cout<<i<<" "<<((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())<<std::endl;                                 
  //  h->Fill(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean()));
  //  if(((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())< 3.14511e-04-3*5.00073e-03 || ((ProjY[i]->GetMean())-(ProjY1[i]->GetMean()))/(ProjY1[i]->GetMean())> 3.14511e-04+3*5.00073e-03) {
  //std::cout<<modid1[i]<<std::endl;
  //  }
  //}
  //}
  ...
  g1->SetTitle(TString("Run2018D1 (Black)") +" & "+ TString("Run2018B (Red)"));
  g2->SetMarkerStyle(8);
  ...
  //h->SetTitle(TString("Run2018D (323363-324420)"));
  //h->SetTitle(TString("Run2018D (324564-325175)"));
  h->SetTitle(TString("weights (D-B)/B"));
  h->GetXaxis()->SetTitle("weights (D-B)/B");
  h->GetYaxis()->SetTitle("entries");
  g1->Draw("AP");
  g2->Draw("PSAME");
  C.Print(Path1+"Graphs_cut_totcount/"+"module_weight_D1_B_testing"+".png");

  g3->Draw("AP");
  C.Print(Path1+"Graphs_cut_totcount/"+"module_weight_(D1-B)_D3_testing"+".png");
  
  //g4->Draw("AP");
  //C.Print(Path1+"Graphs_cut_totcount/"+"module_weight_(D3-B)_D3_vs_zcoordinate_testing"+".png");


  h->Draw();
  C.Print(Path1+"Graphs_cut_totcount/"+"module_weight_(D1-B)_B_histogram_testing"+".png");
