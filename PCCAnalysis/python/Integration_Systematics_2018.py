...
h_ratio=ROOT.TH2F("h_ratio", "PCC/HFOC vs lumi section histogram ", 4433, 0.0, 221654, 4433, 0.94, 1.06)
#h_ratio=ROOT.TH2F("h_ratio", "PCC/HFOC vs lumi section histogram ", 30000, 0.0, 300000, 30000, 0.8, 1.2)
h_ratiovsHF=ROOT.TH2F("h_ratiovsHF", "PCC/HFOC vs HFOC Histogram", 500, 0.0, 25000, 500, 0.8, 1.2)
...
ProjY_ProfX_h_ratio=ROOT.TH1F("ProjY_ProfX","Y Projection of Profile", 200, 0.94, 1.06)
hist_HFOC_perls=ROOT.TH1F("hL"," histogram of HFOC Lumi", 4433, 0, 221654)
#hist_h_ratio = ROOT.TH2F("histo_h_ratio", " Profile vs lumi histogram ", 1080, 0.94, 1.06, 1080, 0.0, 1.5)
...
#with open("/eos/user/a/asehrawa/PCC_PLT_plots/EXPRESS_datasets/Run2018_ZB_test/Run2018/ls.dat", "r") as datFile:
        ...
        for data in datFile:      
                ...
                if float(HFOC_count) !=0 and goodls==True and goodls1==True and goodls2!=True and float(PCC_count)>=6000 and float(HFOC_count)>=6000:
                        ...
                        h_ratio.Fill(lumisec_count+1, float(HFOC_count)/float(PCC_count))  # make TH1 object and fill it with hfoc lumi having same bins as TH2 object, use it to fill 
                        ...
                        hist_HFOC_perls.Fill(lumisec_count+1, float(HFOC_count)*1000000000)
                        ##lumisec_count=lumisec_count+1 
...
ProjY_h_ratio=h_ratio.ProjectionY("Y Projection of PCC/HFOC vs ls", 0, 4433)
...
for i in range(num_bin):
        ProjY_ProfX_h_ratio.Fill(float(ProfX_h_ratio.GetBinContent(int(i))), hist_HFOC_perls.GetBinContent(i))
        #print float(ProfX_h_ratio.GetBinContent(int(i)))
        hist_h_ratio.SetPoint(hist_h_ratio.GetN(), ProfX_h_ratio.GetBinContent(i), hist_HFOC_perls.GetBinContent(i))
        print float(ProfX_h_ratio.GetBinContent(i)), hist_HFOC_perls.GetBinContent(i)
...
PCC_perls.Draw("AP")
C2.Print('/eos/user/a/asehrawa/BRIL-new/'+'PCC_perls_all.png')
...
HFOC_perls.Draw("AP")
C2.Print('/eos/user/a/asehrawa/BRIL-new/'+'HFOC_perls_all.png')
PCC_perls.SetMarkerStyle(20)
HFOC_perls.SetMarkerStyle(20)
PCC_perls.SetMarkerColor(1)
...
#3PCC_perls.GetYaxis().SetRangeUser(0.0001, 100000000)
##HFOC_perls.GetYaxis().SetRangeUser(0.0001, 100000000)
PCC_perls.Draw("AP")
...
legend.SetFillColor(0)
legend.Draw("SAME")
C2.Print('/eos/user/a/asehrawa/BRIL-new/'+'PCC_HFOC_perls_all.png')
