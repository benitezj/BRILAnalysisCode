...
condorqueue=local  #microcentury , workday, testmatch,  local (lxplus jobs in series, not condor), # note in resubmission to change queue need to modify the .sub job file
...
submit(){
	...
	echo "local run: $fullsubmitdir/${run}.sh"
...
make_sh_script(){
    ...
    if [ "$DOCKERIMAGE" != "" ]; then
	echo "$DOCKERIMAGE" >> $fullsubmitdir/${run}.sh
    fi
